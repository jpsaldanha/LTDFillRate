...
ExpShort<-function(x,n){Reduce("rbind",lapply(1:n, function(z){sum(x[z:n]-x[z])/n}))}
...
es0<-lapply(1:D,function(z){ExpShort(TruLTD[,z],n = nX)})
ES<-matrix(0,nrow = nX,ncol = 22)
for(m in 1:D){for(f in 1:(nX-1)){ES[f,m]<-es0[[m]][[f]]}}
...
for(b in 1:D) {
  for (c in U[,1]) {
    ...
    TruROP[b,c]<-TruLTD[hi,b]-(((TS-ES[hi,b])*(TruLTD[hi,b]-TruLTD[lo,b]))/
                                     (ES[lo,b]-ES[hi,b]))
